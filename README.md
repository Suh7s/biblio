...
Book reads and category reads require the shared JWT contract (`token` cookie or Bearer token) and a USER or ADMIN role. Mutations require ADMIN. Auth middleware exports `authenticate` and `authorize(...roles)` from `backend/src/middleware/auth.js`. It attaches `{ _id, id, role }` to `req.user`; other modules can rely on `req.user._id` and `req.user.role`.
